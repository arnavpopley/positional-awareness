...
| `uv run pos sync` | Read-only Groww holdings vs ledger. Uses API key + secret from `.env` (exchanges for a daily access token). Reports missing thesis / not-held / qty-cost drift. Never writes the ledger. |
