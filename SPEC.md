...
- **Local page:** `pos web` — read-only FastAPI on `127.0.0.1`. Optional `config/launchd-web.plist.example` keeps it up. Qty/cost/CMP/return poll Groww every 30s via `/api/quotes` (delayed LTP, not a websocket).
- **Hosted book (optional):** `pos publish` / `pos deploy` push the password-gated book to the existing Vercel project (`positional-awareness` → `temporary-snappy-tuba-wtrnk56.vercel.app`). The HTML is a snapshot of thesis/S/filings; qty/CMP/return refresh from Groww behind the same password. Not a git deploy. Poller still never calls Groww. No orders. Git auto-deploy stays off.
