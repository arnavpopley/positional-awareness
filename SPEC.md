...
- **Hosted snapshot (optional):** `pos publish` / `pos deploy` bake Groww CMP and return into the static copy, then push to the existing Vercel project (`positional-awareness` → `temporary-snappy-tuba-wtrnk56.vercel.app`) behind `PA_SITE_PASSWORD`. Not live. Not a git deploy: the book stays off git. Groww keys stay on the Mac. No orders. GitHub may be connected to that same Vercel project for the URL only; auto-deploy from git stays off.
